...
// global variables
// const int iNumberOfArrayElement = 5;
const int iNumberOfArrayElement = 11444777;
...
{ 
	...
	{ 
		printf("Device Memory allocation is failed for deviceInput array.\n");
		cleanup();
		exit(EXIT_FAILURE);
	}
	...
	// copy data form device array into host array 
	...
	char str[128];
	if (bAccuracy == false)
	sprintf(str, "Comparison for CPU and GPU Vector Addition is not within accuracy of 0.000001 at array index %d", breakValue);
	else
...
{
	// code
	const float fscale = 1.0 / (float)RAND_MAX;
	for (int i = 0; i < len; i++)
	{
		arr[i] = fscale * rand();
	}
