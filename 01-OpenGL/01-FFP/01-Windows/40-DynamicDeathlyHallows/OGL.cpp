...
GLfloat TranslateTriangle_x = -4.0f;
GLfloat TranslateTriangle_y = -4.0f;
bool TriangleFlag = TRUE;

GLfloat TranslateIncircle_x = 4.0f;
GLfloat TranslateIncircle_y = -4.0f;
bool CircleFlag = FALSE;

GLfloat TranslateLine_x = 0.0f;
GLfloat TranslateLine_y = 4.0f;
bool LineFlag = FALSE;
...
void display(void)
{
	...
	glLoadIdentity();
	glTranslatef(TranslateTriangle_x, TranslateTriangle_y, -6.0f);
	glRotatef(angleTriangle, 0.0f, 1.0f, 0.0f);
	drawtriangle();

	glLoadIdentity();
	glTranslatef(TranslateIncircle_x, TranslateIncircle_y, -6.0f);
	glRotatef(angleCircle, 0.0f, 1.0f, 0.0f);
	drawIncircle();

	glLoadIdentity();
	glTranslatef(TranslateLine_x, TranslateLine_y, -6.0f);
	drawVerticleLine();
	SwapBuffers(ghdc);
}
...
void update(void)
{
	...
	if (TriangleFlag)
	{
		TranslateTriangle_x += Offset;
		TranslateTriangle_y += Offset;
		if (TranslateTriangle_x && TranslateTriangle_y >= 0.0f)
		{
			TriangleFlag = FALSE;
			CircleFlag = TRUE;
		}
	...
	if (CircleFlag)
	{
		TranslateIncircle_x -= Offset;
		TranslateIncircle_y += Offset;
		if (TranslateIncircle_x && TranslateIncircle_y >= 0.0)
		{
			CircleFlag = FALSE;
			LineFlag = TRUE;
		} 
	...
	if (LineFlag)
	{
		TranslateLine_y -= Offset;
		if (TranslateLine_y <= 0.0)
		{
			LineFlag = FALSE;
		}
	}
