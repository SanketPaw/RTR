#include <stdio.h>

#define NUM_ROWS 5
#define NUM_COLUMNS 3

int main(void)
{
    //variable declarations
    int iArray_SSP[NUM_ROWS][NUM_COLUMNS];
    int i_SSP, j_SSP;
    int *ptr_iArray_Row_SSP = NULL;
    
    //code
    // *** EVERY ROW OF A 2D ARRAY IS AN INTEGER ARRAY ITSELF COMPRISING OF 'NUM_COLUMNS' INTEGER ELEMENTS ***
    // *** THERE ARE 5 ROWS AND 3 COLUMNS IN A 2D INTEGER ARRAY. EACH OF THE 5 ROWS IS A 1D ARRAY OF 3 INTEGERS. 
    // *** HENCE, EACH OF THESE 5 ROWS THEMSELVES BEING ARRAYS, WILL BE THE BASE ADDRESSES OF THEIR RESPECTIVE ROWS ***
    for (i_SSP = 0; i_SSP < NUM_ROWS; i_SSP++)
    {
        ptr_iArray_Row_SSP = iArray_SSP[i_SSP]; // 'iArray[i]' IS THE BASE ADDRESS OF ith ROW ...
        for (j_SSP = 0; j_SSP < NUM_COLUMNS; j_SSP++)
        *(ptr_iArray_Row_SSP + j_SSP) = (i_SSP + 1) * (j_SSP + 1); // 'ptr_iArray_Row' (That is, 'iArray[i]' Can Be Treated As 1D Array Using Pointers) ...
    }

    printf("\n\n");
    printf("2D Integer Array Elements Along With Addresses : \n\n");
    for (i_SSP = 0; i_SSP < NUM_ROWS; i_SSP++)
    {
        ptr_iArray_Row_SSP = iArray_SSP[i_SSP];
            for (j_SSP = 0; j_SSP < NUM_COLUMNS; j_SSP++)
            {
                printf("*(ptr_iArray_Row + %d)= %d \t \t At Address (ptr_iArray_Row + j) : %p\n", j_SSP, *(ptr_iArray_Row_SSP + j_SSP), (ptr_iArray_Row_SSP + j_SSP));
            }   
        printf("\n\n");
    }
    return(0);
}