...
int main(void)
{
    ...
    {
        iArray_SSP[i_SSP] = (int *)malloc(NUM_COLUMNS_ONE * sizeof(int));
    ...
    for (i_SSP = 0; i_SSP < NUM_ROWS; i_SSP++)
    {
        for (j_SSP = 0; j_SSP < NUM_COLUMNS_ONE; j_SSP++)
        {
    ...
    for (i_SSP = 0; i_SSP < NUM_ROWS; i_SSP++)
    {
        iArray_SSP[i_SSP] = (int *)malloc(NUM_COLUMNS_TWO * sizeof(int));
        if (iArray_SSP[i_SSP] == NULL)
        {
            printf("FAILED TO ALLOCATE MEMORY TO ROW %d OF 2D INTEGER ARRAY !!! EXITTING NOW...\n\n", i_SSP);
            exit(0);
    ...
    for (i_SSP = 0; i_SSP < NUM_ROWS; i_SSP++)
    {
        for (j_SSP = 0; j_SSP < NUM_COLUMNS_TWO; j_SSP++)
        {
            iArray_SSP[i_SSP][j_SSP] = (i_SSP + 1) * (j_SSP + 1);
        }
    }
